...
const UserForm = ({ bool, getId }) => {
  ...
  const handleSelectCountry = (e) => {
    ...
    const filterStates = countriesData.filter(
  ...
  const getCountriesData = async () => {
    ...
    if (inputValue) {
      const filter = countriesData.filter((ele) =>
      ...
      const initialState = json.data.filter(
        (ele) => ele.name.toLowerCase() === selectedCountry.toLowerCase()
      );
  ...
  const handleSave = () => {
    ...
    if (
      ...
      //   bool: false,
      // });
      dispatch(toggleEditPage({ bool: false }));
    } else {
      setErrorPage(true);
    }
  };
  ...
  return (
    <div
      style={bool && { position: "absolute", top: 0, left: 0 }}
      className='w-full h-[98vh] flex flex-col relative items-center justify-center'
    >
      {errorPage && <ErrorForm setErrorPage={setErrorPage} />}
      <div
        style={bool && { background: "#fff" }}
        className='form-element w-96 bg-slate-100 p-5 rounded-lg'
      >
        <h1 className='text-center font-bold text-2xl'>
          {!bool ? "Login Page" : "Edit Page"}
        </h1>
        <form
          className='w-full py-2 h-92 overflow-y-auto form-data flex flex-col gap-1'
          onSubmit={(e) => e.preventDefault()}
        >
          <div className='first-name flex flex-col font-bold'>
            <label htmlFor='f-name'>
              First Name: <span className='text-red-600 text-[18px]'>*</span>{" "}
            </label>
            <input
              type='text'
              name='first_name'
            ...
            {formData.first_name.length >= 1 &&
              formData.first_name.length < 5 && (
                <p className='text-[12px] text-red-600 text-center'>
                  Enter First-name character should be greater than 5
                </p>
              )}
          </div>
          ...
          <div className='last-name flex flex-col font-bold'>
            <label htmlFor='l-name'>
              Last Name: <span className='text-red-600 text-[18px]'>*</span>
            </label>
            <input
              type='text'
              name='last_name'
            ...
            {formData.last_name.length >= 1 &&
              formData.last_name.length < 5 && (
                <p className='text-[12px] text-red-600 text-center'>
                  Enter last-name character should be greater than 5
                </p>
              )}
          </div>
          ...
          <div className='email-id flex flex-col font-bold'>
            <label htmlFor='e-id'>
              Email Id: <span className='text-red-600 text-[18px]'>*</span>
            </label>
            <input
              type='text'
              name='email_id'
          ...
          <div className='address-1 flex flex-col font-bold'>
            <label htmlFor='add1-id'>
              Address 1: <span className='text-red-600 text-[18px]'>*</span>
            </label>
            <textarea
              name='address_1'
              ...
              rows='2'
            ></textarea>
            {formData.address_1.length >= 1 &&
              formData.address_1.length < 10 && (
                <p className='text-[12px] text-red-600 text-center'>
                  Please fill More information about the address
                </p>
              )}
          </div>
          ...
          <div className='flex justify-between'>
            <div className='countries-data w-24'>
              ...
              <p className='text-orange-500 font-bold font-mono'>
                {selectedCountry}
              </p>
            </div>
            ...
            <div className='states-data w-24'>
              ...
              <p className='text-green-600 font-bold font-mono'>
                {State || (data && data.State) || "No State"}
              </p>
            </div>
